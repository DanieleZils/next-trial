...
export default function About() {
  ...
  useEffect(() => {
    const check = () => {
      const contScroll = bodyRef.current ? bodyRef.current.scrollTop : 0;
      setScrolled(window.scrollY > 40 || contScroll > 40);
    };
    const cont = bodyRef.current;
    window.addEventListener("scroll", check, { passive: true });
    if (cont) cont.addEventListener("scroll", check, { passive: true });
    return () => {
      window.removeEventListener("scroll", check);
      if (cont) cont.removeEventListener("scroll", check);
    };
  }, []);
  ...
  return (
    <main className="relative w-full bg-background pt-20">
      <div className="grid grid-cols-1 md:grid-cols-2">
        ...
        {/* Biography — fixed header + scrolling body */}
        <div className="flex flex-col md:h-[calc(100vh-5rem)]">
          {/* Fixed header */}
          <div className="shrink-0 px-6 pt-12 pb-6 md:px-14 lg:px-20 md:pt-20 md:pb-8">
            <span className="block text-[11px] uppercase tracking-[0.3em] text-primary mb-4">
              Biography
            </span>
            <h1 className="font-serif text-4xl md:text-5xl font-medium leading-tight text-balance text-foreground">
          ...
          <div className="relative flex-1 min-h-0">
            <div
              ref={bodyRef}
              className="h-full overflow-y-auto px-6 md:px-14 lg:px-20 pb-24 md:pb-28 pt-[8vh] md:pt-[10vh]"
            >
            ...
            {/* Bottom fade */}
            <div
              className={`pointer-events-none absolute inset-x-0 bottom-0 h-36 bg-gradient-to-t from-background via-background/80 to-transparent transition-opacity duration-700 ${
            ...
            {/* Scroll invitation */}
            <div
              className={`pointer-events-none absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 transition-opacity duration-700 ${
                scrolled ? "opacity-0" : "opacity-100"
              }`}
            >
